...
type ReqBody = {
  ...
  email: string;
  password: string;
  nextStep?: 'MFA_SETUP' | 'SOFTWARE_TOKEN_MFA';
  mfaCode?: string;
  session?: string;
};
 ...
 * ユーザーサインイン
 */
export const signIn = ash(async (req: Request<unknown, unknown, ReqBody>, res: Response<ResBody | NextStepResBody>) => {
  const { tenantId, email, password, nextStep, mfaCode, session } = req.body;
